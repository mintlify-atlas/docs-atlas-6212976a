...
| YAML | `.yaml` | Requires the `yaml` extra (`pip install pai[yaml]`). |
...
<Warning>
  YAML support requires an optional dependency. Install it with `pip install pai[yaml]` before using a `.yaml` config file.
</Warning>
