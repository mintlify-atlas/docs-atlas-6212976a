...
```
{base}/{element-type}/{name}/states/{property}
```

The intermediate `states` segment comes from `MQTT_STATES_TOPIC` (default: `states`).

<Note>
In the source code, the state topic prefix is built as `{MQTT_BASE_TOPIC}/{MQTT_STATES_TOPIC}/{element-topic}`. Element names are sanitized before use as topic segments.
</Note>
...
| Purpose | Config keys | Full default topic |
|---|---|---|
| Raw JSON events | `MQTT_EVENTS_TOPIC`, `MQTT_RAW_TOPIC` | `paradox/events/raw` |
| Interface / PAI status | `MQTT_INTERFACE_TOPIC` | `paradox/interface/pai_status` |
| Availability | `MQTT_INTERFACE_TOPIC` | `paradox/interface/availability` |
| Challenge (HMAC auth) | `MQTT_STATES_TOPIC`, `MQTT_CHALLENGE_TOPIC` | `paradox/states/challenge` |
| Command status | `MQTT_INTERFACE_TOPIC`, `MQTT_COMMAND_STATUS_TOPIC` | `paradox/interface/command_status` |
...
Publish to `paradox/control/partitions/{name}`:
...
Publish to `paradox/control/outputs/{name}`:
...
Publish to `paradox/panic/{type}/{partition}/{userid}` to trigger a panic. `{type}` is the panic type code accepted by the panel.
...
Enable `MQTT_PUBLISH_COMMAND_STATUS = True` to have PAI publish accepted/refused messages for each command to `paradox/interface/command_status` (QoS 2, retained).
